...
def _parse_args():
    ...
    parser.add_argument(
        "--stop-amortizing",
        action="store_true",
        help="Use if you want to stop amortizing when your debt quota is <4.5 and your debt is <50% of the house's value.",
    )
...
def main() -> None:
    ...
    print(f"House price: {house_price} | Principal: {principal} | Down payment: {down_payment} | Interest: {interest}")
    ...
    while month_counter / 12 < args.years:
        ...
        interest_payment = (debt * (interest / 100)) / 12
        tot_interest_payment += interest_payment

        if month_counter % 12 == 0:
            if debt / house_price > 0.7:
                amortization_percentage = 2
            elif debt / house_price > 0.5:
            ...
            print(
                f"The debt / house price ratio is: {(debt/house_price)*100:.1f}%. ",
                end="",
            )
            debt_quota = debt / yearly_income
            ...
            if amortization_percentage == 0 and args.stop_amortizing:
                print(
                    "Your debt is down to 50% of the house's value and your debt quota is <4.5. You have opted to stop amortizing!"
                )
                break

        loan_payment = (principal * (amortization_percentage / 100)) / 12
        ...
        debt -= loan_payment
        print(
            f"Month: {month_counter:3} | Interest: {interest_payment:5.0f}, Amortization ({amortization_percentage}%): {loan_payment:5.0f} | Monthly payment: {interest_payment + loan_payment + args.running_cost:7.0f} | Remaining debt: {debt:7.0f}, Total interest: {tot_interest_payment:7.0f}"
        )
